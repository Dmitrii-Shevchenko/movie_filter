...
attr_reader :link, :title, :year, :country, :release, :genre, :time, :rate, :producer, :actors
  ...
  def self.create(mov,col)
    case mov[2].to_i
      when (1900..1945)
          AncientModule::Ancient.new(mov,col)
      when (1945..1968)
          ClassicModule::Classic.new(mov,col)
      when (1968..2000)
          ModernModule::Modern.new(mov,col)
      when (2000..Date.today.year)
          NewModule::New.new(mov,col)
      else
         MovieModule::Movie.new(mov,col)
    end
  end
